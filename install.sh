...
DOT_DRY_RUN="${DOT_DRY_RUN:-${DRY_RUN:-0}}"
...
if command -v vim >/dev/null 2>&1; then
  if is_dry; then
    log "[dry-run] vim -E -es -u $REPO/vimrc -c 'silent! helptags ALL' -c 'qa!'"
  else
...
if command -v nvim >/dev/null 2>&1; then
  if is_dry; then
    log "[dry-run] nvim --headless -u $REPO/init.lua -c 'silent! helptags ALL' -c 'qa!'"
  else
...
if command -v make >/dev/null 2>&1; then
  ...
  if is_dry; then
    log "[dry-run] (cd $REPO && make build)"
  else
